...
function authFromHeader(req: express.Request): User | null {
...
app.post("/api/auth/refresh", async (req, res) => {
  ...
  let payload: any;
  try {
    payload = jwt.verify(rt, JWT_REFRESH_SECRET) as { sub: string; role: Role };
  } catch {
    return res.status(401).json({ message: "Invalid refresh token" });
  }

  const u = db.users.get((payload as any).sub);
...
app.patch("/api/products/:id", requireAuth, requireAdmin, (req, res) => {
  ...
  const now = new Date().toISOString();
  const next: Product = { ...p, ...body.data, updatedAt: now };
